...
public class MeshPainter : MonoBehaviour
{
	...
	private void PaintLine(Vector2 from, Vector2 to)
	{
		...
		{
			Vector2 lerped = Vector2.Lerp(from, to, (float)i / (float)steps);
			PaintAt(lerped);
		}
	}
	...
	{
		brushMat.SetVector("_BrushUV", new Vector4(uv.x, uv.y, 0f, 0f));
	...
	private float GetPaintedPercentage()
	{
		...
		RenderTexture.active = paintRT;
		tex.ReadPixels(new Rect(0f, 0f, paintRT.width, paintRT.height), 0, 0);
		tex.Apply();
		...
		float percent = painted / (float)pixels.Length * 100f;
		Object.Destroy(tex);
		return percent;
	}
